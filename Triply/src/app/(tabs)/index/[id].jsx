import { Text, View, TouchableOpacity, Image, ScrollView, ActivityIndicator, TextInput, Alert } from "react-native";
...
export default function Publicacao() {
  ...
  const adicionarComentario = async () => {
    ...
    try {
      ...
      await api.post("/comentarios", comentarioData);
  ...
  if (error || !publicacao) {
    return (
      <SafeAreaView style={publiStyle.container}>
        <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
          <Text style={[publiStyle.fonte]}>{error || "Publicação não encontrada."}</Text>
          <TouchableOpacity
            onPress={() => router.back()}
    ...
    <SafeAreaView style={publiStyle.container}>
      <ScrollView contentContainerStyle={publiStyle.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={publiStyle.header}>
          <TouchableOpacity onPress={() => router.back()} style={publiStyle.buttonBack}>
        ...
        <View style={publiStyle.cardPublicacao}>
          <View style={publiStyle.linhaPerfil}>
            ...
            <View style={publiStyle.infoPerfil}>
              <Text style={[publiStyle.nomePerfil, homeStyle.fonte]}>{publicacao.nomeUsuario}</Text>
              <Text style={[publiStyle.nomePerfilData, homeStyle.fonte]}>{publicacao.data}</Text>
            </View>
          </View>
          ...
          {publicacao.imagem && (
            <Image style={publiStyle.imagePost} source={{ uri: publicacao.imagem }} />
          )}

          <View style={publiStyle.botaoCurtir}>
            <TouchableOpacity style={publiStyle.botaoCurtir}>
              <Ionicons name="heart-outline" size={24} color="#000000" />
              <Text style={[publiStyle.fonte]}>{publicacao.curtidas}</Text>
            </TouchableOpacity>

            <TouchableOpacity style={publiStyle.botaoComentar}>
              <Ionicons name="chatbubble-ellipses-outline" size={24} color="#000000" />
              <Text style={[publiStyle.fonte]}>{publicacao.comentariosCount}</Text>
            </TouchableOpacity>

            <TouchableOpacity style={publiStyle.botaoSalvar}>
              <Ionicons name="bookmark-outline" size={24} color="#000000" />
              <Text style={[publiStyle.fonte]}>{publicacao.salvamentos}</Text>
            </TouchableOpacity>
          </View>
        </View>

        <Text style={[publiStyle.subtitle, publiStyle.fonte]}>Comentários ({comentarios.length})</Text>

        {comentarios.map((comentario) => (
          <View key={comentario.id} style={publiStyle.cardComentario}>
            <View style={publiStyle.linhaPerfil}>
              <View style={publiStyle.cardPerfil}>
                <Image
              ...
              <View style={publiStyle.infoPerfil}>
                <Text style={[publiStyle.nomePerfil, homeStyle.fonte]}>{comentario.nomeUsuario}</Text>
                <Text style={[publiStyle.nomePerfilData, homeStyle.fonte]}>{comentario.data}</Text>
              </View>
            </View>
        ...
        <View style={publiStyle.cardComentario2}>
          ...
          <TouchableOpacity style={publiStyle.botaoPost} onPress={adicionarComentario}>
